...
export function FlightDetails() {
  ...
  const flightMap: Record<string, FlightInfo> = {
    ...
    'FR5935': {
      ...
      duration: '1h 35m',
      pnrList: [
        { code: 'V64LYT', label: '小花・春香・麗安・許振宏' },
  ...
  return (
    <div className="mt-20 px-4 pb-44 max-w-xl mx-auto space-y-4">
      ...
      <section className="bg-surface-container-lowest rounded-2xl p-4 sm:p-5 border border-outline-variant/15 shadow-xs space-y-3">
          ...
          <div className="w-[56px] min-w-[56px] shrink-0 flex flex-col items-center justify-center text-center mx-auto">
            {f.duration && (
              <div className="flex flex-col items-center justify-center font-mono text-[13px] sm:text-[15px] font-bold text-outline leading-[1.1] uppercase">
                {f.duration.replace(/\(.*\)/, '').trim().toUpperCase().split(/\s+/).filter(Boolean).map((p, idx) => (
                  <span key={idx}>{p}</span>
                ))}
              </div>
            )}
            <Plane className={`text-primary rotate-90 ${f.duration ? 'mt-2' : ''}`} size={14} fill="currentColor" />
          </div>
